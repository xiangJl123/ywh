    ...
    <a-tabs defaultActiveKey="1" @change="callback">
      <a-tab-pane tab="已迁入" key="1">
        <div class="button-warp">
          <div class="right">
            ...
            <a-input-search placeholder="备注查询" @search="onSearch" />
            <a-button type="link">
              重置
            </a-button>
          </div>
        ...
        >
          ...
          <a slot="action" slot-scope="text,record">
            <span @click="showDetail(text,record)">查看</span> 
      ...
      <a-tab-pane tab="未通过" key="4">
        ...
        >
          <!-- 按钮位置 -->
          <a slot="action" slot-scope="text,record">
            <span @click="delOwner(text,record)">删除</span> 
          </a>
        </a-table>
      </a-tab-pane>
    </a-tabs>
    <!-- 业主信息 -->
    <edit-owner :open="showEdit" @on-cancel="backOwner"></edit-owner>
    <!-- // 迁出说明 -->
...
//  已迁入 ，待审核
const columns1 = [
    {title: '业主姓名', dataIndex: 'key', width: '10%', scopedSlots: { customRender: 'name' }},
    {title: '性别', dataIndex: 'money', width: '10%', scopedSlots: { customRender: 'name' }},
    ...
    {title: '备注', dataIndex: 'name3', width: '15%', scopedSlots: { customRender: 'name' }},
...
import { EditOwner } from "./components"
import {  OutDes} from "../hold/components"
export default {
  components: { EditOwner },
  data(){
    return {
      data,
      columns1,
